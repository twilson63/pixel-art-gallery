    ...
    <div>
      <header className="pa2 bg-light-gray mb4 flex justify-around">
        <h3 className="tc sans-serif black-80">
          <a
            href="https://app.pixelfun.xyz"
            className="link ba br2 pa2 bg-orange white dim"
          >
            Play
          </a>
        </h3>
        <h3 className="tc sans-serif black-80">
          <Link to="/" className="link ba br2 pa2 bg-orange white dim">
            Search
          </Link>
        </h3>
        <h3 className="tc sans-serif black-80">
...
const enhance = compose(
  ...
  lifecycle({
    componentDidMount() {
      db.get(this.props.match.params.id).then(doc => {
        this.props.dispatch({
          type: 'SET_DOC',
          payload: doc
        })
